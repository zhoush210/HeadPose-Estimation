...
# 注释7个标记点
def annotate_landmarks7(im,landmark7):
    '''
    注释7个标记点
    :param im: 输入图片
    :return: 带有7个标记点的图片
    '''
    img = im.copy()
    for point in landmark7:
        point = (int(point[0]), int(point[1]))
        cv2.circle(img, point, 3, color=(255, 255, 255))
    return img
...
def get_landmark7(img):
    ...
    landmark7 = np.array([  # 取出68中所需的7个detection object
...
while (cap.isOpened()):
    ...
    ret, points_2D = get_landmark7(img)  # dlib检测7个特征点
    ...
    if ret == 1:  # 检测到人脸7个特征点
        ...
        img_with_line7_points = annotate_landmarks7(img_with_line,points_2D)

        euler_angle_str = 'Pitch:{}, Yaw:{}, Roll:{}'.format(theta[0], theta[1], theta[2])

        fps = 1 / (time.time() - start_time)

        cv2.putText(img_with_line7_points, euler_angle_str, (0, 20), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 1)
        cv2.putText(img_with_line7_points, "FPS : " + str(int(fps)), (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255),1)
        cv2.imshow("output", img_with_line7_points)
        cv2.waitKey(1)
    else:  # 未检测到人脸特征点
        fps = 1 / (time.time() - start_time)
